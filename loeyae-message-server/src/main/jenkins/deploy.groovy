node {
    ...
    stage("deploy") {
        ...
        def imageTag = "hub.bys.cd:30339/library/loeyae_message_server:${buildId}"
        ...
        withCredentials([dockerCert(credentialsId: 'docker-client', variable: 'DOCKER_CERT_PATH')]) {
            try {
                sh """
                    docker rmi \$(docker images | grep "loeyae_message_server" | awk "{print \$3}") -f
                """
            } catch(exc) {
        ...
        def source = "loeyae-message-server/src/main/jenkins/loeyae-message-server.yml"
    ...
    stage("Post") {
        if (currentBuild.currentResult == 'SUCCESS') {
            echo "sucess"
            build job: 'loeyae_message_server-deploy-prod', parameters: [text(name: 'IMAGE_TAG', value: BUILD_ID)], propagate: false, quietPeriod: 9, wait: false
        }
    }
}